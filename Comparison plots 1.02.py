...
##The following reads the type of data (rainfall, snow, stream discharge,etc.) and responds accordingly.
    ...
    plt.title(cst.metadata.define_model_run(Compared_file)+'\n' + title, fontsize = 18)
...
def run():
    ...
    ref_file_list = list(reference_file)
...
##    AltScenarioList = list(cst.metadata.AltScenarios[1:6])

## If you only want to plot the scenario indicated in the master file, 
##   uncomment this line:
    AltScenarioList = [str(compared_file[0]).split("_")[-2]]  #List of length 1
## DO NOT DELETE THE TEXT ABOVE HERE ^^^^^

#    AltScenarioList = list(cst.metadata.AltScenarios[1:6])
    RefCase = cst.metadata.AltScenarios[0]

    # Make the plots.
    for plot_number in range(total_number_of_plots):
        if ToBePlotted[plot_number]:
            for AltScenario in AltScenarioList:
                compared_file_name = ref_file_list[plot_number].replace(RefCase, AltScenario)
                compare(
